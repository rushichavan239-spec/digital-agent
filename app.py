...
def fetch_leads(query, location):
    ...
    try:
        ...
        for index, result in enumerate(results[:8]):
            ...
            if title_tag:
                name = title_tag.text.strip().split('|')[0]
...
if st.button("Launch Engine"):
    if target_industry and target_location:
        with st.spinner("🤖 Agent is scanning the market..."):
            data = fetch_leads(target_industry, target_location)
            
        if data:
            st.success(f"🎉 Generated {len(data)} premium leads!")
            ...
            for item in data:
                st.markdown(f"""
                <div class="card">
                    <strong style="color: #FFFFFF;">🏢 {item['Business Name']}</strong><br>
